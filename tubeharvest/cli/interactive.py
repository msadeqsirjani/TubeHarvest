...
def validate_configuration(config: Dict[str, Any]) -> bool:
    ...
    if config['audio_only'] and config['video_only']:
        ui.show_error("Cannot use both audio-only and video-only options simultaneously")
        return False
    ...
    try:
        ...
        ui.show_error(f"Cannot write to output directory {config['output_dir']}: {str(e)}")
        return False
        ...
        while True:
            try:
                ...
                if success:
                    ...
                    ui.show_warning("Some downloads may have failed. Check the output above.")
                
                # Ask if user wants to download something else
                if not ui.ask_retry("Would you like to download another video/playlist?"):
                    break
