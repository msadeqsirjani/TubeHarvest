...
from .utils import (
    validate_url, is_playlist_url, create_output_directory, 
    format_file_size, save_metadata, sanitize_filename,
...
class TubeHarvestDownloader:
    ...
    def _get_format_options(self) -> Dict[str, Any]:
        ...
        if self.audio_only:
            if self.format_type == "mp3":
                ...
                format_options["format"] = f"bestaudio[ext={self.format_type}]/bestaudio/best"
                
        # Handle video-only downloads
        elif self.video_only:
            if self.quality == "best":
                format_options["format"] = f"bestvideo[ext={self.format_type}]/bestvideo/best"
            else:
    ...
    def _get_ydl_opts(self) -> Dict[str, Any]:
        ...
        if self.custom_filename:
            ...
            ydl_opts['outtmpl'] = os.path.join(self.output_dir, f'{safe_filename}.%(ext)s')
    ...
    def _download_single_video(self, video_url: str = None) -> bool:
        ...
        try:
            ...
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                ...
                if not info:
                    print_error(f"Could not retrieve video information for: {url_to_download}")
                    return False
    ...
    def _download_playlist(self) -> bool:
        ...
        try:
            ...
            with yt_dlp.YoutubeDL(base_opts) as ydl:
                ...
                if self.custom_filename:
                    playlist_dir = os.path.join(self.output_dir, sanitize_filename(self.custom_filename))
                else:
                    playlist_dir = os.path.join(self.output_dir, sanitize_filename(playlist_title))
                ...
                video_urls = [entry.get('url') for entry in videos if entry and entry.get('url')]
                
                print_info(f"Starting download of {len(video_urls)} videos with {self.max_workers} workers...")
                ...
                with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
                    ...
                    for i, future in enumerate(as_completed(future_to_url)):
                        ...
                        try:
                            ...
                            print_info(f"Overall progress: {progress:.1f}% ({i+1}/{len(video_urls)})")
                        except Exception as e:
                ...
                # Final report
                print_success(f"Playlist download complete. Successfully downloaded: {self.total_downloads}/{video_count} videos")
